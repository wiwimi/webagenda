...
/**
 * Provides functionality to add, update and delete notifications in the
 * database.
 * 
 * @author dann
 * @version 0.01.00
 * @license GPL 2 FIXME: Need fuzzy search for type? other fields?
 */
public class NotificationBroker extends Broker<Notification>
    ...
    public static NotificationBroker getBroker()
        {
        if (nbrok == null)
            nbrok = new NotificationBroker();
        return nbrok;
        }
    
    /**
     * Method that contains all throwable conditions when attempting to access
     * broker methods of EmployeeBroker. TODO: Do all proper permission checks
     * for broker methods
     * 
    ...
    private PermissionLevel checkPermissions(Employee target)
        ...
        PermissionLevel[] pla = persistence.PermissionBroker.getBroker().get(target.getLevel(),
                target.getVersion(), target);
        if (pla == null)
            {
            throw new InvalidPermissionException(
                    "(Warning) PermissionLevels not found for caller Employee; Ensure Employee exists");
            }
    ...
    public boolean delete(Notification deleteObj, Employee caller) throws DBException,
        ...
        try
            ...
            if (result != 1)
                throw new DBException("Failed to delete notification, result count incorrect: " +
                        result);
            else
                success = true;
            }
    ...
    /*
     * Use negative numbers in a searchTemplate object to search all items in
     * that category (non-Javadoc)
    ...
    public Notification[] get(Notification searchTemplate, Employee caller) throws DBException,
        ...
        {
        if (caller == null)
            throw new DBException("Cannot get Notifications if user is null");
        ...
        if (searchTemplate == null)
            ...
            select = String
                    .format(
                            ...
                            (searchTemplate.getNotificationID() >= 0 ? searchTemplate
                                    .getNotificationID() : "%"),
    ...
    protected Notification[] parseResults(ResultSet rs) throws SQLException
        ...
        if (rs.last())
            ...
            for (int i = 0; i < resultCount && rs.next(); i++)
                {
                Notification note = new Notification(rs.getInt("notificationID"), rs
                        .getInt("senderID"),
    ...
    @Override
    public boolean update(Notification oldNotification, Notification updateObj, Employee caller)
            throws DBException,
            DBDownException, InvalidPermissionException
        {
        if (updateObj == null)
        ...
        // FIXED: (Daniel Kettle) Removed sentTime from update notification as
        // a) time is restamped upon being written to db, and b)
        // attempting to set a timeStamp will cause an update to fail because
        // the notification being updated doesn't have one.
        String update = String
                .format(
                        ...
                        updateObj.getRecipientID(), (updateObj.isViewed() == true ? 1 : 0),
                        updateObj.getMessage(),
