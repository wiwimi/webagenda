...
public class PositionBroker extends Broker<Position>
    ...
    /**
     * Constructor that initializes a Broker Connection Monitor object to
     * monitor old connections.
     */
    ...
    /**
     * Returns the PositionBroker object, initializing it if necessary, to
    ...
    public boolean create(Position createObj, Employee caller) throws DBException,
         ...
         * use will be caught, and turned into our own DBAddException, so this
        ...
        try
            ...
            conn.getConnection().setAutoCommit(false); // Temporarily while
    ...
    /*
     * ( Important! All Deletions should use internal method object deletePos,
     * not parameter deleteObj. deleteObj is used to specify the desired
     * Position NAME whereas deletePos will be the returned results of this
     * broker object for that position name. A user may not know skills
     * beforehand, and then we would have to do a skill array check. )
     * (non-Javadoc)
    ...
    public boolean delete(Position deleteObj, Employee caller) throws DBException,
        ...
        /*
         * Delete goes through three steps: 1) Grab Position from get(), as we
         * do not know if deleteObj is a true representation of the Position to
         * delete. - There can only be one Position deleted at a time - Skillset
         * that comes with Position may be set to null when delete is requested.
         * - If no skills exist, we delete Position right away and commit. 2) We
         * must grab Skills from SkillBroker to ensure they exist. Skills are
         * found in the POSSKILL table and compared against those in the
         * parameter. - If not enough skills are found that are specified, there
         * is something wrong with Position object and it can be deleted. (or we
         * handle it otherwise) - If the number of skills required is found,
         * delete and commit. 3) Perform an orphan skill check. Grab all unique
         * skill entries in POSSKILL and compare against count in SKILL table.
         * If !=, check each SKILL entry for an entry in POSSKILL. If one does
         * not exist, delete that SKILL entry. Do for all entries.
         */
        ...
        try
            ...
            Position[] deletePos = PositionBroker.getBroker().get(
                    new Position(deleteObj.getName()), caller);
            ...
            if (deletePos[0].getPos_skills() == null)
                ...
                {
                // Skills are not null, so we must grab them and utilize them.
                // Step 2.
                ...
                for (Skill s : deletePos[0].getPos_skills())
                    ...
                    try
                        ...
                        if (temp.length != 1)
                            throw new DBException(
                                    "Skill does not exist or too many instances retrieved");
                        posSkills[i] = temp[0];
                        temp = null;
                        }
                        ...
                        conn.getConnection().setAutoCommit(true);
                        throw new DBException(
                                "The Position object pointed to a Skill that does not exist and therefore is invalid.");
                        // FIXME: have it skip this skill, maybe init a variable
                        // array with skill that don't exist
                        }
                    catch (Exception E)
                        {
                        conn.getConnection().rollback();
                        conn.getConnection().setAutoCommit(true);
                        conn.setAvailable(true);
                        throw new DBException("An Exception occured that will not be dealt with " +
                                E);
                        }
                    finally
                        {
                        ...
                        deletePos[0].setPos_skills(posSkills); // This sets the
                        // Position to
                        // utilize the
                        // only available
                        // skills that
                        // exist at this time, enabling the Position to be
                        // deleted even if imaginary skills somehow exist. (how
                        // they
                        // would is beyond me at this time, unless db was
                        // modified by outside sources.)
                        }
                    // At this point, if a variable was to be set that lets
                    // method know that a skill doesn't exist that should, we
                    // can handle it here.
                ...
                if (result != 1)
                    ...
                    conn.getConnection().rollback();
                    conn.getConnection().setAutoCommit(true);
                    throw new DBException("Failed to delete position, result count incorrect: " +
                            result);
                    }
    ...
    public Position[] get(Position searchTemplate, Employee caller) throws DBException,
        ...
        if (searchTemplate.getDescription() != null)
            comp = comp +
                    (searchTemplate.getDescription() != null ? (comp.equals("") ? "" : " AND ") +
        ...
        try
            ...
            for (Position p : foundPositions)
                ...
                select = String.format(
                        "SELECT * FROM `WebAgenda`.`POSSKILL` WHERE positionName = '" +
                                p.getName() + "';");
                stmt = conn.getConnection().createStatement();
                searchResults = stmt.executeQuery(select);
                
                // This statement parses Skills from the Position results then
                // ensures they exist in Skill table.
                // Skills are set to the position in the loop (no exceptions
                // means worked properly) which is returned.
    ...
    protected Position[] parseResults(ResultSet rs) throws SQLException
        ...
        if (rs.last())
            ...
            for (int i = 0; i < resultCount && rs.next(); i++)
                {
                pos = new Position(rs.getString("positionName"), rs
                        .getString("positionDescription"),
                        null);
                posList[i] = pos;
                }
            }
    ...
    /**
     * Method to parse and return all the skills that would be associated to
     * positions that would be returned.
     * 
     * @param rs ResultSet of an executed Statement object to grab positions and
     *            skills from the position-skill table.
     * @return Skill[] array of Skills that match up with positions being
     *         returned.
     * @throws SQLException
     */
    ...
    /**
     * Method returns a completed list of skills (adds descriptions, does not
     * throw exception if executed without error.) - Checks to ensure that
     * skills exist in skill broker.
     * 
     ...
     * @throws DBException
     */
    private Skill[] ensureSkillsExist(Skill[] input, Employee caller) throws DBException,
            DBDownException
        {
        if (input == null)
            return null;
    ...
    /**
     * Checks whether the desired objects exist as expected before modifying
     * them. The new Position object can be null when specifying a delete(),
     * otherwise is considered an update(). A null old position will throw an
     * error.
     * 
     * @param oldP
     * @param newP
     ...
     * @throws SQLException
     */
    public void raceCondition(Position oldP, Employee caller) throws DBException, DBDownException,
            SQLException
        {
        if (oldP == null)
            throw new DBException("Unable to check race condition on unavailable Position");
        // We grab the original object (oldP) and put it into a temporary
        // variable.
        Position[] temp = PositionBroker.getBroker().get(oldP, caller);
        // No object found, race condition failed.
        if (temp.length > 1)
            throw new DBChangeException("Too many rows for Position to be updated");
        else if (temp.length < 1)
            throw new DBChangeException("Position to check for race condition not found.");
        // Configure SQL statement to get the number of Skills associated with a
        // Position
        String update = "SELECT COUNT(*) FROM `WebAgenda`.`POSSKILL` WHERE positionName = '" +
                temp[0].getName() + "';";
        
        // Declare lengths of skills in positions so we don't always have to
        // check for null values.
        int oldPosL = -1;
        if (oldP.getPos_skills() != null)
            oldPosL = oldP.getPos_skills().length;
        
        // Grab a connection to retrieve old object as seen in the database.
        DBConnection conn = this.getConnection();
        Statement stmt = conn.getConnection().createStatement();
        ResultSet rs = stmt.executeQuery(update); // It's now stored in result
        // set
        ...
        if (temp[0].getPos_skills() != null)
            ...
            if (count_value != temp[0].getPos_skills().length)
                ...
                throw new DBChangeException(
                        "This Position has been modified and cannot be changed. Please update new Position object");
                }
            }
        stmt = conn.getConnection().createStatement();
        ...
        // Check all skills -- in exact order -- from old position and position
        // from db that should BE old
        int i = 0;
        rs.beforeFirst();
        while (rs.next())
            {
        ...
        for (; i < oldPosL; i++)
            ...
            if (!oldP.getPos_skills()[i].getName().equals(skills[i]))
                ...
                throw new DBChangeException(
                        "Skills do not match up with old position for update. Position may have been modified.");
                }
            i++;
            }
        }
    ...
    public boolean update(Position oldPos, Position updateObj, Employee caller) throws DBException,
        ...
        try
            {
            raceCondition(oldPos, caller); // will throw exception if old object
            // is changed
            ...
            if (oldPos.getPos_skills() != null)
                oldPosL = oldPos.getPos_skills().length;
            if (updateObj.getPos_skills() != null)
                newPosL = updateObj.getPos_skills().length;
            String update = null;
            ...
            if (oldPosL > newPosL)
                ...
                for (int i = 0; i < oldPosL; i++)
                    {
                    if (!oldPos.getPos_skills()[i].getName().equals(
            ...
            if (posChanged)
                ...
                update = String
                        .format(
                                ...
                                updateObj.getName(),
                                (updateObj.getDescription() == null ? "NULL" : "'" +
                                        updateObj.getDescription() + "'"),
                                oldPos.getName(),
                                (oldPos.getDescription() == null ? "IS NULL" : "= '" +
                                        oldPos.getDescription() + "'"));
                ...
                if (i != 1)
                    ...
                    conn.setAvailable(true);
                    throw new DBChangeException(
                            "Position requried a description update that failed. Check original Position data.");
                    }
                }
            ...
            if (skillsChanged)
                ...
                if (i != 1)
                    ...
                    conn.setAvailable(true);
                    throw new DBChangeException(
                            "Position not found, may have been changed or deleted by another user.");
                    }
                ...
                // Last insert took care of position object. This one takes care
                // of POSSKILL, so we must do insert for each skill associated.
                ...
                if (newPosL > 0)
                    ...
                    for (i = 0; i < updateObj.getPos_skills().length; i++)
                        ...
                        if (stmt.executeUpdate(update) != 1)
                            ...
                            conn.setAvailable(true);
                            throw new DBChangeException(
                                    "Could not insert Position/Skill entry as it may already exist.");
                            }
