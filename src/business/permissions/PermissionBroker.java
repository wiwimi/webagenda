...
public class PermissionBroker extends Broker<PermissionLevel>
    ...
    /**
     * Returns the only instance of PermissionBroker that exists to return
     * read-only copies of permission and permission level objects.<br>
     * <br>
     * Actual setting of permissions require a Validation object that is
     * determined to be acceptable or not (thrown exception if invalid) to
     * actually set permissions. It also applies to getting permission levels,
     * but even if permissions are returned as read-only, it's not so much a
     * security risk.
     * 
    ...
    public static PermissionBroker getBroker()
        ...
        if (broker_permissions == null)
            ...
            Logging.writeToLog(Logging.INIT_LOG, Logging.NORM_ENTRY,
                    "Permission Broker initialized");
            }
        return broker_permissions;
        }
    
    /**
     * Method that contains all throwable conditions when attempting to access
     * broker methods of PermissionBroker.
     * 
     ...
     */
    private PermissionLevel checkPermission(PermissionLevel plevel, Employee caller)
        ...
        if (caller.getLevel() < plevel.getLevel())
            ...
            {
            pl = get(PermissionAccess.getAccess().getLevel(caller.getLevel(), caller.getVersion()),
                    caller)[0];
            if (pl == null)
                throw new InvalidPermissionException(
                        "No matches for caller's Permission Level found");
            if (pl.getLevel_permissions().getTrusted() <= plevel.getLevel())
                {
                throw new InvalidPermissionException(
                        "User is not trusted to the level required to perform this action");
                }
            else
                {
    ...
    /**
     * Public method that allows other Brokers to determine what the trusted
     * level of a specified employee is.
     * 
     * @param caller
    ...
    public int trustedLevelOf(Employee caller) throws DBException, DBDownException
        {
        return get(PermissionAccess.getAccess().getLevel(caller.getLevel(), caller.getVersion()),
                caller)[0].getLevel_permissions().getTrusted();
        }
    
    @Override
    public boolean create(PermissionLevel createObj, Employee caller) throws DBException,
        ...
        PermissionLevel pl = checkPermission(createObj, caller); // This will
                                                                 // throw any
                                                                 // exceptions
                                                                 // due to
                                                                 // invalid
        ...
        if (pl.getLevel_permissions().isCanChangePermissions())
            throw new PermissionViolationException(
                    "User is not authorized to Create a PermissionLevel");
        ...
        String insert = "INSERT INTO `WebAgenda`.`PERMISSIONSET` " +
                ...
                " VALUES ('" +
                createObj.getLevel() + ", " +
                createObj.getVersion() + "', " +
                createObj.getLevel_permissions().isCanEditSchedule() + ", " +
                createObj.getLevel_permissions().isCanReadSchedule() + ", " +
                createObj.getLevel_permissions().isCanReadOldSchedule() + ", " +
                createObj.getLevel_permissions().isCanManageEmployees() + ", " +
                createObj.getLevel_permissions().isCanViewResources() + ", " +
                createObj.getLevel_permissions().isCanChangePermissions() + ", " +
                createObj.getLevel_permissions().isCanReadLogs() + ", " +
                createObj.getLevel_permissions().isCanAccessReports() + ", " +
                createObj.getLevel_permissions().isCanRequestDaysOff() + ", " +
                createObj.getLevel_permissions().getMaxDaysOff() + ", " +
                createObj.getLevel_permissions().isCanTakeVacations() + ", " +
                createObj.getLevel_permissions().getMaxVacationDays() + ", " +
                createObj.getLevel_permissions().isCanTakeEmergencyDays() + ", " +
                createObj.getLevel_permissions().isCanViewInactiveEmployees() + ", " +
                createObj.getLevel_permissions().isCanSendNotifications() + ", " +
                createObj.getLevel_permissions().getTrusted() + ")";
    ...
    public boolean delete(PermissionLevel deleteObj, Employee caller) throws DBException,
        ...
        // This will throw any exceptions due to invalid permission access
        PermissionLevel pl = checkPermission(deleteObj, caller); 
        if (pl.getLevel_permissions().isCanChangePermissions())
            throw new PermissionViolationException(
                    "User is not authorized to Delete a PermissionLevel");
        
        boolean success = false;
        String delete = "DELETE FROM `WebAgenda`.`PERMISSIONSET` WHERE plevel = " +
                deleteObj.getLevel() + " AND pversion = '" + deleteObj.getVersion() + "';";
        try
            {
            DBConnection conn = this.getConnection();
            Statement stmt = conn.getConnection().createStatement();
            ...
            if (result != 1)
                throw new DBException(
                        "Failed to delete permission level " + deleteObj.getLevel() +
                                deleteObj.getVersion() + ": " +
                                result);
            success = true;
            }
        catch (SQLException e)
    ...
    public PermissionLevel[] get(PermissionLevel searchTemplate, Employee caller)
        ...
        if (searchTemplate.getLevel() >= 0)
            ...
            comparisons = comparisons + "WHERE plevel = " + searchTemplate.getLevel() +
                    " AND pversion = '" + searchTemplate.getVersion() + "'";
            }
        else
            {
            ...
            // Check if permissions object is null: find all if that is the
            // case.
            if (perm == null)
                {
                ...
                comparisons = comparisons + " AND canManageEmployee = " +
                        perm.isCanManageEmployees();
                // Can view resources
                comparisons = comparisons + " AND canViewResources = " + perm.isCanViewResources();
                // Can change permissions (up to current level - 1)
                comparisons = comparisons + " AND canChanagePermissions = " +
                        perm.isCanChangePermissions();
                // Can read log files via a built in viewer (logs hosted on the
                // server)
                ...
                // Max Vacation Days
                comparisons = comparisons +
                        (perm.getMaxVacationDays() >= 0 ? " AND maxVacationDays = " +
                                perm.getMaxVacationDays() : "");
                // Can Take Emergency Days Off
                comparisons = comparisons + " AND canTakeEmergencyDays = " +
                        perm.isCanTakeEmergencyDays();
                // Can View Inactive Employees
                comparisons = comparisons + " AND canViewInactiveEmps = " +
                        perm.isCanViewInactiveEmployees();
                // Can Send Notifications
                comparisons = comparisons + " AND canSendNotifications = " +
                        perm.isCanSendNotifications();
                // Trusted Employee
    ...
    /**
     * Method that finds all permission levels where the level is below the
     * searchTemplate's level.
     * 
     * @param int level
    ...
    public PermissionLevel[] getAllBelow(int level) throws DBException, DBDownException
        ...
        if (level <= 0)
            throw new DBException("Unable to search for Permission Levels with a negative value");
        // Create sql select statement from permission level object.
        String select = "SELECT * FROM `WebAgenda`.`PERMISSIONSET` WHERE plevel < " + (level) +
                " ORDER BY plevel;";
    ...
    /**
     * An alterate get method that returns get() results using level and version
     * as opposed to a permission level object. This is mainly for convenience,
     * as methods that return PermissionLevels are protected to prevent access.
     * 
     ...
     * @throws DBDownException
     */
    public PermissionLevel[] get(int level, char version, Employee caller) throws DBException,
            DBDownException
        {
    ...
    /*
     * In the very nature of this method call, the primary key or
     * PermissionLevel value itself cannot be changed once created. It must be
     * deleted and re-created. The front-end can (aka should) do this
     * automatically. (non-Javadoc)
     * @see persistence.Broker#update(business.BusinessObject)
     */
    @Override
    public boolean update(PermissionLevel oldObj, PermissionLevel updateObj, Employee caller)
        ...
        // This will throw any exceptions due to invalid permission access
        PermissionLevel pl = checkPermission(updateObj, caller); 
        ...
        String set = "SET "; // This string is modified to contain fields being
                             // set and their changed values
        String where = " WHERE plevel = " + updateObj.getLevel() + " AND pversion = '" +
        ...
        try
            ...
            if (result != 1)
                throw new DBException(
                        "Failed to update permission level " + updateObj.getLevel() +
                                updateObj.getVersion() + ": " +
                                result);
            success = true;
            }
        ...
        if (rs.last())
            ...
            for (int i = 0; i < resultCount && rs.next(); i++)
                ...
                Permissions p = PermissionAccess.getAccess().getCustomPermission(
                        rs.getBoolean("canEditSched"), rs.getBoolean("canReadSched"),
                        rs.getBoolean("canReadOldSched"),
                        rs.getBoolean("canManageEmployee"), rs.getBoolean("canViewResources"),
                        rs.getBoolean("canChangePermissions"), rs.getBoolean("canReadLogs"),
                        rs.getBoolean("canAccessReports"),
                        rs.getBoolean("canRequestDaysOff"), rs.getInt("maxDaysOff"),
                        rs.getBoolean("canTakeVacations"), rs.getInt("maxVacationDays"),
                        rs.getBoolean("canTakeEmergencyDays"),
                        rs.getBoolean("canViewInactiveEmps"),
                        rs.getBoolean("canSendNotifications"),
                        rs.getInt("trusted"));
                ...
                try
                    ...
                    {
                    // By having nothing in catch, PermissionLevel that cannot
                    // be set (as it is not null) will be ignored.
