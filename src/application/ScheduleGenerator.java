...
public class ScheduleGenerator
	...
	public static Schedule generateSchedule(ScheduleTemplate template,
		...
		try
			...
			PreparedStatement selectStrict = conn.getConnection().prepareStatement(
			...
			PreparedStatement selectRecom = conn.getConnection().prepareStatement(
							"SELECT * FROM `EMPLOYEE` "
									+ "WHERE prefPosition = ? "
									+ "AND empID NOT IN ("
									+ "SELECT e.empID FROM `EMPLOYEE` e JOIN `SHIFTEMP` se ON e.empID = se.empID "
			...
			for (ShiftTemplate st : template.getShiftTemplates().toArray())
				...
				for (ShiftPosition shiftPos : st.getShiftPositions().toArray())
					...
					if (strictMatches == null ||
						...
						{
						// Not enough employees returned to fill the shift positions.
						...
						// Add recommended employees to recommended shift, if any are
						// returned.
						if (recMatches != null)
							{
							...
							// Add recommended employees to shift arrayList to be
							// returned.
